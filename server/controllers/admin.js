...
const adminLogin = TryCatch(async(req,res, next)=>{
    ...
    // const adminSecretKey = process.env.ADMIN_SECRET_KEY || "secret"
    ...
    return res.status(200).cookie("chats-admin-token", token,{
        ...cookieOptions,
        maxAge: 1000 * 60 * 15,
    })
    .json({
    ...
    const adminLogout = TryCatch(async(req,res, next)=>{
        return res.status(200).cookie("chats-admin-token", "",{
            ...cookieOptions,
            maxAge: 0,
        })
        .json({
...
export {allUsers, allChats, allMessages, getDashboardStats, adminLogin, adminLogout, adminData}