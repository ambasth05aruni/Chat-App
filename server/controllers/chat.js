...
if(members.length <  2) return next(
        new ErrorHandler("Group chat must have atleast 3 members", 404)
    )
    ...
    emitEvent(res, ALERT, allMembers,`Welcome to ${name} group`);
...
const otherMember =  getOtherMember(members, req.user)
    ...
    const getMyGroups = TryCatch(async(req, res, next)=>{
        const chat = await Chat.find({members:req.user, groupChat:true, creator:req.user})
        .populate(
            "members",
            "name avatar"
        )
...
if(!members || members.length<1 )
    ...
    const leaveGroup = TryCatch(async(req, res, next)=>{
        ...
        if(remainingMembers.length < 3){
            return next(new ErrorHandler("Groups must have atleast 3 members", 400))
        }
        ...
        emitEvent(req, ALERT, chat.members,
            `User ${user.name} have left the group`);
...
const messageForDB ={contents :"", attachments, sender:me._id, chat:chatId}
...
//here we have to delete all msgs and attachments too from cloudinary
...
export {newGroupChat, getMyChat, 
    getMyGroups, addMembers, removeMembers, leaveGroup,
